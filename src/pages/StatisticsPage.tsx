...
export function StatisticsPage({ onBack, participants, totalDays }: StatisticsPageProps) {
  ...
  const totalMales = participants.filter(p => p.data.gender === 'male').length;
  const totalFemales = participants.filter(p => p.data.gender === 'female').length;

  // Get all unique attendance dates
  const allAttendanceDates = Array.from(
    new Set(participants.flatMap(p => p.attendanceDays))
  ).sort();

  const filteredDates = allAttendanceDates;
  const totalAttendanceRecords = filteredDates.reduce((sum, date) => {
    return sum + participants.filter(p => p.attendanceDays.includes(date)).length;
  }, 0);

  // Average attendance per day
  const avgAttendancePerDay = filteredDates.length > 0
    ? Math.round(totalAttendanceRecords / filteredDates.length)
    : 0;
  ...
  const genderData = [
    { id: 'male', name: 'ذكور', value: totalMales, color: '#3B82F6' },
    { id: 'female', name: 'إناث', value: totalFemales, color: '#EC4899' }
  ];
  ...
  const stageData = Object.entries(educationStageLabels).map(([key, label]) => {
    ...
    return {
      id: key,
      name: label,
      total: stageParticipants.length,
      ذكور: males,
      إناث: females
    };
  }).filter(s => s.total > 0);

  // Attendance over time (last 14 days or available days)
  const attendanceTimelineData = filteredDates.slice(-14).map(date => {
  ...
  const pointsDistributionData = pointsRanges.map(({ range, min, max }) => ({
    range,
    count: participants.filter(p => p.points >= min && p.points <= max).length
  })).filter(d => d.count > 0);

  // Attendance rate statistics
  const attendanceRates = participants.map(p => ({
    rate: totalDays > 0 ? (p.attendanceDays.length / totalDays) * 100 : 0
  }));
  ...
  const highAttendance = participants.filter(p =>
    (p.attendanceDays.length / totalDays) * 100 >= 80
  ).length;
      ...
      <div className="p-4 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-card rounded-xl p-4 shadow-sm border border-border">
            ...
            <div className="text-center">
              <div className="text-2xl mb-1 text-primary">{totalParticipants}</div>
          ...
          <div className="bg-card rounded-xl p-4 shadow-sm border border-border">
            ...
            <div className="text-center">
              <div className="text-2xl mb-1 text-green-600">{totalAttendanceRecords}</div>
          ...
          <div className="bg-card rounded-xl p-4 shadow-sm border border-border">
            ...
            <div className="text-center">
              <div className="text-2xl mb-1 text-blue-600">{avgAttendancePerDay}</div>
              <div className="text-xs text-muted-foreground">متوسط الحضور اليومي</div>
            </div>
          </div>
          ...
          <div className="bg-card rounded-xl p-4 shadow-sm border border-border">
            ...
            <div className="text-center">
              <div className="text-2xl mb-1" style={{ color: 'var(--secondary)' }}>{avgPoints}</div>
        ...
        <div className="bg-card rounded-xl p-5 shadow-sm border border-border">
          ...
          <div className="grid grid-cols-2 gap-3 mt-4">
            <div className="text-center p-3 bg-blue-50 rounded-lg">
              <div className="text-2xl font-bold text-blue-600">{totalMales}</div>
              <div className="text-xs text-blue-700">ذكور</div>
            </div>
            <div className="text-center p-3 bg-pink-50 rounded-lg">
              <div className="text-2xl font-bold text-pink-600">{totalFemales}</div>
        ...
        {attendanceTimelineData.length > 0 && (
          <div className="bg-card rounded-xl p-5 shadow-sm border border-border">
            ...
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={attendanceTimelineData}>
                <CartesianGrid strokeDasharray="3 3" key="grid" />
                <XAxis dataKey="date" style={{ fontSize: '10px' }} key="xaxis" />
                <YAxis style={{ fontSize: '11px' }} key="yaxis" />
                <Tooltip key="tooltip" />
                <Legend key="legend" />
                <Line
        ...
        <div className="bg-card rounded-xl p-5 shadow-sm border border-border">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
              ...
              <span className="text-lg font-bold text-primary">{avgAttendanceRate.toFixed(1)}%</span>
            </div>
            <div className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
              <span className="text-sm">مشاركين بحضور عالي (80%+)</span>
              <span className="text-lg font-bold text-green-600">{highAttendance}</span>
            </div>
            <div className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
              <span className="text-sm">إجمالي أيام الحضور المسجلة</span>
              <span className="text-lg font-bold text-blue-600">{filteredDates.length}</span>
            </div>
            <div className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
              <span className="text-sm">إجمالي النقاط الموزعة</span>
              <span className="text-lg font-bold" style={{ color: 'var(--secondary)' }}>{totalPoints}</span>
            </div>
          </div>
        </div>
        ...
        {Object.entries(educationStageLabels).map(([stageKey, stageLabel]) => {
          ...
          const stageTotalPoints = stageParticipants.reduce((sum, p) => sum + p.points, 0);
          const stageAvgPoints = Math.round(stageTotalPoints / stageParticipants.length);
          const stageMaxPoints = Math.max(...stageParticipants.map(p => p.points));
          const stageMinPoints = Math.min(...stageParticipants.map(p => p.points));
                    ...
                    .map((participant, index) => (
                      <div key={participant.id} className="flex items-center justify-between p-2 bg-muted/30 rounded-lg">
                        ...
                        <div className="text-right">
                          <div className="font-bold text-sm text-green-600">{participant.attendanceDays.length} يوم</div>
                          <div className="text-xs text-muted-foreground">
                            {((participant.attendanceDays.length / totalDays) * 100).toFixed(0)}%
                          </div>
                        </div>
                      </div>
                    ))}
