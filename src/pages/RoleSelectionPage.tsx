...
export function RoleSelectionPage({ onSelectRole }: RoleSelectionPageProps = {}) {
  ...
  const { setViewerRole } = useFestivalStore();

  const handleSelectRole = (role: 'servant' | 'student') => {
    setViewerRole(role);
    if (onSelectRole) {
      onSelectRole(role);
    } else {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full max-w-md">
        <button onClick={() => handleSelectRole('servant')} className="bg-card flex flex-col items-center justify-center p-8 rounded-2xl shadow-lg border-2 border-transparent hover:border-primary active:scale-95 transition-all">
        ...
        <button onClick={() => handleSelectRole('student')} className="bg-card flex flex-col items-center justify-center p-8 rounded-2xl shadow-lg border-2 border-transparent hover:border-secondary active:scale-95 transition-all">
